//! Snort 3 IDS rule generator for Echos profiles.
//!
//! Generates experimental Snort 3 rules from profile definitions.
//! Review and tune before deploying in production.

use crate::profiles::{Protocol, TrafficProfile};

/// SID base — Echos uses 9_100_000 range to distinguish Snort rules from Suricata (9_000_000).
const SID_BASE: u64 = 9_100_000;

/// Generate a Snort 3 rules string for the given profile.
...
pub fn generate(profile: &TrafficProfile, profile_index: usize) -> String {
    ...
    lines.push(format!(
        "# Echos - Snort 3 Rules for profile: {}",
        profile.name
    ));
...
fn build_http_rule(profile: &TrafficProfile, sid: u64) -> String {
    ...
    // Snort 3 uses http_header sticky buffer; search for the full User-Agent header line.
    if let Some(ua) = profile.custom_headers.get("User-Agent") {
        opts.push("http_header;".to_string());
        opts.push(format!("content:\"User-Agent: {}\";", ua));
    } else {
        opts.push("http_method;".to_string());
        opts.push("content:\"GET\";".to_string());
    }
    if let Some(ct) = profile.custom_headers.get("Content-Type") {
        opts.push("http_header;".to_string());
        opts.push(format!("content:\"Content-Type: {}\";", ct));
    }
...
fn build_dns_rule(profile: &TrafficProfile, target: &str, sid: u64) -> String {
    ...
    // Snort 3 lacks a dns.query sticky buffer; match on raw UDP payload — tune before production.
    ...
    format!("alert udp $HOME_NET any -> any 53 ({})", opts.join(" "))
}
...
mod tests {
    ...
    fn test_generate_sid_present() {
        ...
        let rules = generate(&p, 0);
        assert!(
            rules.contains("sid:9100001"),
            "expected 'sid:9100001' in:\n{}",
            rules
        );
    }
    ...
    #[test]
    fn test_http_header_used_for_ua() {
        let mut p =
            TrafficProfile::new("Cobalt", "http://127.0.0.1:8080", 10, 20.0, Protocol::Http);
        p.add_header("User-Agent", "Mozilla/5.0");
        let rules = generate(&p, 0);
        assert!(rules.contains("http_header"));
        assert!(rules.contains("User-Agent: Mozilla/5.0"));
    }
